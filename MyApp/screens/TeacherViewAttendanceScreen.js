...
const TeacherViewAttendanceScreen = () => {
    ...
    const fetchAttendanceData = async () => {
        try {
            const response = await axios.get('http://172.20.10.10:5000/view_student_attendance', {
                params: {
            ...
            console.log("recieved",response.data); // Log the response data
            setFilteredData(response.data.attendance_records || []); // Make sure to set the correct field
        ...
        <View style={styles.container}>
            ...
            {filteredData && filteredData.length > 0 ? (
                filteredData.map((student, index) => {
                    ...
                    return (
                        <View key={student.student_database_id || index} style={styles.studentContainer}>
                            <Text style={styles.studentName}>
                                {student.student_name} - ID: {student.student_database_id}
                            </Text>
                            <View style={styles.attendanceContainer}>
                                <View style={styles.attended}>
                                    <Text style={styles.attendedText}>Attended {student.attended_classes}/{student.total_classes}</Text>
                                </View>
